...
public class ReportDetailServiceImpl extends RemoteServiceServlet implements ReportDetailService
{
	...
	public ModelData getModel(ReportType report, ModelData params)
	{
		...
		} else if (ReportType.MedicalCamp.equals(report))
		{
			...
			List breakupOfTreatments = (List) DataUtil
					.executeQuery("select t.referral1 as referral, sum(t.cnt) as screened, case t.medicines when 'YES' then 1 else 0 end case as medicines,case t.caseclosed when 'YES' then 1 else 0 end case as caseclosed, case t.surgerycase when 'YES' then 1 else 0 end case surgerycase from ( select cpd.referral1,count(cpd.referral1) as cnt, cpd.medicines, cpd.caseclosed, cpd.surgerycase, cpd.emergency from camppatientdetail cpd join referraltype rt on (cpd.referral1 = rt.name ) group by referral1, medicines, caseclosed, surgerycase, emergency union all select cpd.referral2, count(cpd.referral2) as cnt, cpd.medicines, cpd.caseclosed, cpd.surgerycase, cpd.emergency from camppatientdetail cpd join referraltype rt on (cpd.referral2 = rt.name ) group by referral2, medicines, caseclosed, surgerycase, emergency union all select cpd.referral2,count(*) as cnt, cpd.medicines, cpd.caseclosed, cpd.surgerycase, cpd.emergency from camppatientdetail cpd where cpd.referral1 is null and cpd.referral2 is null group by referral2, medicines, caseclosed, surgerycase, emergency ) t group by t.referral1, t.medicines, t.caseclosed, t.surgerycase, t.emergency");
			Map<String, ModelData> referral2Model = new HashMap<String, ModelData>();
			for (Object object : breakupOfTreatments)
			{
				ModelData modelTemp;
		...
		} else if (ReportType.School.equals(report))
		{
			...
			List breakupOfTreatments = (List) DataUtil
					.executeQuery("select t.referral1 as referral, sum(t.cnt) as screened, case t.medicines when 'YES' then 1 else 0 end case as medicines,case t.caseclosed when 'YES' then 1 else 0 end case as caseclosed, case t.surgerycase when 'YES' then 1 else 0 end case surgerycase from ( select spd.referral1,count(spd.referral1) as cnt, spd.medicines, spd.caseclosed, spd.surgerycase,spd.emergency from schoolpatientdetail spd join referraltype rt on (spd.referral1 = rt.name ) group by referral1, medicines, caseclosed, surgerycase, emergency union all select spd.referral2, count(spd.referral2) as cnt, spd.medicines, spd.caseclosed, spd.surgerycase, spd.emergency from schoolpatientdetail spd join referraltype rt on (spd.referral2 = rt.name ) group by referral2, medicines, caseclosed, surgerycase, emergency union all select spd.referral2,count(*) as snt, spd.medicines,spd.caseclosed, spd.surgerycase, spd.emergency from schoolpatientdetail spd where spd.referral1 is null and spd.referral2 is null group by referral2, medicines, caseclosed, surgerycase, emergency ) t group by t.referral1, t.medicines, t.caseclosed, t.surgerycase, t.emergency");
			Map<String, ModelData> referral2Model = new HashMap<String, ModelData>();
			for (Object object : breakupOfTreatments)
			{
				ModelData modelTemp;
