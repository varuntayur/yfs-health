...
public class ReportDetailServiceImpl extends RemoteServiceServlet implements ReportDetailService
{
	...
	private void generateSchoolScreeningReport(ModelData params, ModelData model)
	{
		...
		model.set("locationsCount", DataUtil.executeQuery("select count(*) from schoolscreeningdetail sd join locality lo on sd.localityid = lo.localityid join schoolscrdet_patdet spd on spd.schScrId = sd.schoolscreeningdetailid where sd.screeningdate >= " + fromDate + " and sd.screeningdate <= " + toDate));
	...
	private void generateEventsReport(ModelData params, ModelData model)
	{
		...
		List<ModelData> results = genMedCampSummary();

		results.addAll(genSchoolScreeningSummary());
	...
	{
		List<ModelData> results = new ArrayList<ModelData>();
	...
	private List<ModelData> genSchoolScreeningSummary()
	{
		...
		if (lstObjsSchool != null)
			results = extractRowData(mapId2Docs, mapId2Volunteers, lstObjsSchool);

		return results;
	}

	private List<ModelData> genMedCampSummary()
	{
		Map<String, List<String>> mapId2Docs = null;
		Map<String, List<String>> mapId2Volunteers = null;
		List<ModelData> results = new ArrayList<ModelData>();

		List<Object> lstObjsCamp = (List<Object>) DataUtil.executeQuery("select cd.screeningdate as date , 'Camp Screening', pt.name as eventType, cd.address as eventLocation, count(*) as noscreened, cd.campscreeningdetailid, d.name from campscreeningdetail cd join locality lo on cd.localityid = lo.localityid join campscrdet_patdet cpd on cpd.camscrid = cd.campscreeningdetailid join processtype pt on pt.processtypeid = cd.processtypeid join campscrdet_doct csdd on cd.campscreeningdetailid = csdd.camscrid join doctor d on d.doctorid = csdd.docid group by cd.screeningdate,pt.name, cd.address,cd.campscreeningdetailid,d.name");
		List<Object> lstVolunteers = (List<Object>) DataUtil.executeQuery("select cd.screeningdate as date , 'Camp Screening', pt.name as eventType, cd.address as eventLocation, count(*) as noscreened, cd.campscreeningdetailid, v.name from campscreeningdetail cd join locality lo on cd.localityid = lo.localityid join campscrdet_patdet cpd on cpd.camscrid = cd.campscreeningdetailid join processtype pt on pt.processtypeid = cd.processtypeid join campscrdet_volunt csdv on cd.campscreeningdetailid = csdv.camscrid join volunteer v on v.volunteerid = csdv.volid group by cd.screeningdate,pt.name, cd.address,cd.campscreeningdetailid, v.name");
		...
		if (lstObjsCamp != null)
			results = extractRowData(mapId2Docs, mapId2Volunteers, lstObjsCamp);
