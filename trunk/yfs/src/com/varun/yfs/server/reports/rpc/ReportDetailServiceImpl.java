...
public class ReportDetailServiceImpl extends RemoteServiceServlet implements ReportDetailService
{
	...
	public ModelData getModel(ReportType report, ModelData params)
	{
		...
		if (ReportType.Clinic.equals(report))
		{
			...
			List breakupOfTreatments = (List) DataUtil.executeQuery("select t.referral,sum(t.count1) as screened, case t.medicines when 'YES' then 1 else 0 end case as medicines,case t.caseclosed when 'YES' then 1 else 0 end case as caseclosed, case t.surgerycase when 'YES' then 1 else 0 end case surgerycase from (select cph.referral1 as referral, count(cph.referral1) as count1, cph.medicines, cph.caseclosed, cph.surgerycase from clinicpatientdetail cpd join clipatdet_clipathis cpdcph on cpd.clipatdetid = cpdcph.clipatdetid join clinicpatienthistory cph on cph.clipathisid = cpdcph.clipathisid where cpd.clinicid =" + clinicId
